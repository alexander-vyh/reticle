...
// --- Alias feedback operations ---

/**
 * Confirm that a mentioned_name is a valid alias for an entity.
 * Adds the alias to entity_aliases and attributes all matching unattributed facts.
 *
 * @param {Object} db - org-memory database
 * @param {Object} opts
 * @param {string} opts.entityId - The entity this alias belongs to
 * @param {string} opts.mentionedName - The mentioned_name to confirm as an alias
 * @param {string} [opts.sourceFactId] - The fact that triggered the review (optional)
 * @returns {{ aliasId: string, attributedCount: number }}
 */
function confirmAlias(db, { entityId, mentionedName, sourceFactId }) {
  const normalized = mentionedName.normalize('NFC');
  const now = Math.floor(Date.now() / 1000);

  return db.transaction(() => {
    // Add alias (idempotent via INSERT OR IGNORE)
    const id = crypto.randomUUID();
    db.prepare(
      'INSERT OR IGNORE INTO entity_aliases (id, entity_id, alias, alias_source, confirmed_at) VALUES (?, ?, ?, ?, ?)'
    ).run(id, entityId, normalized, 'user_confirmed', now);

    // If the alias already existed, update confirmed_at
    db.prepare(
      'UPDATE entity_aliases SET confirmed_at = ?, alias_source = ? WHERE entity_id = ? AND alias = ? AND confirmed_at IS NULL'
    ).run(now, 'user_confirmed', entityId, normalized);

    // Attribute all matching unattributed facts
    const result = db.prepare(
      'UPDATE facts SET entity_id = ? WHERE entity_id IS NULL AND LOWER(mentioned_name) = LOWER(?)'
    ).run(entityId, normalized);

    return { aliasId: id, attributedCount: result.changes };
  })();
}

/**
 * Reject a mentioned_name as NOT being an alias for an entity.
 * Records a negative example in rejected_aliases.
 *
 * @param {Object} db - org-memory database
 * @param {Object} opts
 * @param {string} opts.entityId - The entity this alias does NOT belong to
 * @param {string} opts.mentionedName - The mentioned_name being rejected
 * @param {string} [opts.sourceFactId] - The fact that triggered the review (optional)
 */
function rejectAlias(db, { entityId, mentionedName, sourceFactId }) {
  const normalized = mentionedName.normalize('NFC');
  const now = Math.floor(Date.now() / 1000);
  const id = crypto.randomUUID();

  db.prepare(
    'INSERT OR IGNORE INTO rejected_aliases (id, entity_id, alias, rejected_at, source_fact_id) VALUES (?, ?, ?, ?, ?)'
  ).run(id, entityId, normalized, now, sourceFactId || null);
}
 ...
 * @returns {Object[]} Array of alias rows
 */
function getAliases(db, entityId) {
  return db.prepare('SELECT * FROM entity_aliases WHERE entity_id = ?').all(entityId);
}

/**
 * Get all rejected aliases for an entity.
 * @param {Object} db - org-memory database
 * @param {string} entityId
 * @returns {Object[]} Array of rejected alias rows
 */
function getRejectedAliases(db, entityId) {
  return db.prepare('SELECT * FROM rejected_aliases WHERE entity_id = ?').all(entityId);
}
