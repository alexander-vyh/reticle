...
function insertRawMessage(db, { source, sourceId, channelId, channelName, authorId, authorName, content, threadId, occurredAt, metadata }) {
  const now = Math.floor(Date.now() / 1000);
  const id = crypto.randomUUID();
  ...
  db.prepare(
    `INSERT INTO raw_messages (id, source, source_id, channel_id, channel_name,
      author_id, author_name, content, thread_id, occurred_at, metadata, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    id, source, sourceId, channelId || null, channelName || null,
    authorId || null, authorName || null, content, threadId || null,
