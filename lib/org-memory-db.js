...
function initDatabase(dbPath) {
  ...
  // Migration: add confirmed_at column to entity_aliases
  try {
    db.exec('ALTER TABLE entity_aliases ADD COLUMN confirmed_at INTEGER');
  } catch (e) {
    if (!e.message.includes('duplicate column')) throw e;
  }

  // Migration: rejected_aliases table for negative examples
  db.exec(`
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS idx_rejected_aliases_unique ON rejected_aliases(entity_id, alias);
    CREATE INDEX IF NOT EXISTS idx_rejected_aliases_alias ON rejected_aliases(alias);
  `);
