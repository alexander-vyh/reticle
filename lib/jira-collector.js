...
function classifyTicket(issue) {
  ...
  // Capability: DWDEV project is almost always capability
  if (issue.key && issue.key.startsWith('DWDEV-')) {
    return 'capability';
  }
...
async function collectJiraResolved(db, jiraToken, weekStart, weekEnd) {
  ...
  const jql = `project in (DWDEV, DWS) AND resolved >= "${startDate}" AND resolved <= "${endDate}" ORDER BY resolved DESC`;
