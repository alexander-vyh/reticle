...
NAME SCRUBBING:
- NEVER include individual team member names. Attribute work to teams, not people.
- Say "the team", "the Infrastructure team", "the Security team" — never "Alice" or "Bob".
- If source data includes names, silently drop them from the output.
...
- Include individual employee names (this is a strict requirement)`;
...
function formatWeeklySummaryFallback(curatedData) {
  // Handle both old shape (teams/executiveSummary) and new shape (sections/gaps)
  const sections = curatedData.sections || [];
  const lines = [];

  lines.push('## Digital Workplace');
  lines.push('');
  lines.push('### Executive Summary');
  lines.push('Digital Workplace operations remained stable this week with no employee-impacting disruptions. Overall risk posture remains unchanged heading into next week.');
  lines.push('');
  lines.push('### Team Notes');

  if (sections.length > 0) {
    // New shape: sections array with { teamKey, teamName, items }
    for (const section of sections) {
      lines.push('');
      lines.push(`#### ${section.teamName}`);
      if (section.items.length === 0) {
        ...
        for (const item of section.items) {
          const text = item.summary || item.observation || item.signal || JSON.stringify(item);
          lines.push(`- ${text}`);
        }
      }
    }
  ...
  } else if (curatedData.teams) {
    // Legacy shape fallback
    function renderTeam(label, team, defaultMsg) {
      lines.push('');
      lines.push(`#### ${label}`);
      if (!team) { lines.push(defaultMsg); return; }
    ...
    renderTeam('Corporate Systems Engineering', curatedData.teams.cse, 'Routine identity lifecycle operations continued.');
    renderTeam('Desktop Support', curatedData.teams.desktop, 'Normal operational activity; no notable trends or employee-impacting issues requiring attention this week.');
    renderTeam('Security (Platform & Endpoint)', curatedData.teams.security, 'Routine operations continued.');
  }
...
async function narrateWeeklySummary(curatedData, previousNotes) {
  ...
  // Primary: API client (gateway or direct — fastest and most reliable)
  ...
  if (client) {
    try {
      ...
      // With extended thinking, response has thinking + text blocks — find the text block
