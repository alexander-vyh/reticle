...
/**
 * Determine priority from the original urgency signal AND age.
 * Age is a staleness indicator, not a criticality escalator.
 * A batch email never becomes critical just because it's old.
 */
function unrepliedPriority(ageSeconds, urgency) {
  if (urgency === 'urgent') {
    // Genuinely urgent: stays high/critical
    if (ageSeconds > 48 * 3600) return 'critical';
    if (ageSeconds > 24 * 3600) return 'high';
    return 'high';
  }
  // Batch/normal: age determines staleness, not criticality
  if (ageSeconds > 7 * 86400) return 'normal';
  if (ageSeconds > 3 * 86400) return 'normal';
  if (ageSeconds > 24 * 3600) return 'low';
  return 'low';
}

function awaitingPriority(ageSeconds) {
  if (ageSeconds > 7 * 86400) return 'normal';
  if (ageSeconds > 3 * 86400) return 'low';
  return 'low';
}
...
function collectFollowups(db, accountId) {
  ...
  for (const conv of pending) {
    ...
    const urgency = conv.urgency || (conv.metadata && JSON.parse(conv.metadata || '{}').urgency) || 'normal';
    const priority = unrepliedPriority(age, urgency);
  ...
  const resolved = reticleDb.getResolvedToday(db, accountId);
...
function collectEmail(db, accountId, { vipEmails = [] } = {}) {
  ...
  if (vipEmails.length > 0) {
    const pending = reticleDb.getPendingResponses(db, accountId, { type: 'email' });
    for (const conv of pending) {
...
function collectO3(db, accountId) {
  ...
  const thisWeekSessions = reticleDb.getWeeklyO3Summary(db, weekStart, now)
  ...
  const upcomingSessions = reticleDb.getWeeklyO3Summary(db, now, nextWeekEnd)
    .filter(s => s.account_id === accountId);
...
/**
 * Collect open commitments and action items from the org-memory knowledge graph.
 * Surfaces items where the user or their team have unresolved obligations.
 * @param {Object} orgMemDb - org-memory database
 * @param {Object} [opts]
 * @param {number} [opts.staleDays=7] - Days after which an open item is considered stale
 * @returns {Object[]} DigestItem array
 */
function collectCommitments(orgMemDb, { staleDays = 7 } = {}) {
  const now = Math.floor(Date.now() / 1000);
  const staleThreshold = now - (staleDays * 24 * 3600);
  const items = [];

  // Query open event facts: commitments, action items, risks
  const openFacts = orgMemDb.prepare(`
    SELECT f.id, f.attribute, f.value, f.valid_from, f.source_message_id,
           e.canonical_name, e.id as entity_id
    FROM facts f
  ...
  for (const fact of openFacts) {
    const ageSeconds = now - fact.valid_from;
    const isStale = fact.valid_from < staleThreshold;

    // Determine priority based on type and age
    let priority;
    if (fact.attribute === 'raised_risk') {
    ...
    const labels = {
      ...
      asked_to: 'Action item',
      raised_risk: 'Open risk',
      decided: 'Decision (unresolved)',
    };

    items.push(createDigestItem({
      collector: 'commitments',
      observation: `${labels[fact.attribute]}: ${fact.canonical_name} — ${fact.value}`,
      reason: `Open ${fact.attribute.replace('_', ' ')} from ${formatAge(ageSeconds)} ago`,
      authority: 'org-memory knowledge graph',
      consequence: isStale
        ? 'This item may have been forgotten — verify if it is still relevant'
        : 'Track for follow-up',
      sourceType: 'knowledge-graph',
      category: fact.attribute === 'raised_risk' ? 'risk' : 'followup',
      priority,
      ageSeconds,
      counterparty: fact.canonical_name,
...
function collectMeetings(db) {
  ...
  for (const meeting of meetings) {
    ...
    if (meeting.summary) observation += ` — ${meeting.summary.substring(0, 80)}`;
    if (hasFlags && isUnreviewed) observation += ` [${flagged.length} item${flagged.length > 1 ? 's' : ''} need review]`;
    items.push(createDigestItem({
      collector: 'meeting', observation,
      ...
      consequence: hasFlags ? 'Review speaker assignments and flagged items in Reticle.' : 'No action needed. Summary is available.',
      sourceType: 'meeting',
      category: isUnreviewed && hasFlags ? 'meeting-needs-review' : 'meeting-summarized',
      priority: isUnreviewed && hasFlags ? 'high' : 'normal',
      entityId: meeting.id, observedAt: meeting.start_time
    }));
  }
  return items;
}

module.exports = { collectFollowups, collectEmail, collectO3, collectCalendar, collectCommitments, collectMeetings };
