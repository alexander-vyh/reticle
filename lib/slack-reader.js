...
// Prefer user token (can read all channels the user is in) over bot token
// (can only read channels the bot has been invited to)
const SLACK_TOKEN = config.slackUserToken || config.slackBotToken;
...
// Tier 2 rate limit for search.messages (20 requests/min)
const searchLimiter = createRateLimiter(20, 20 / 60);
...
async function listConversations({ types = 'public_channel,private_channel' } = {}) {
  const all = [];
  let cursor;
  do {
...
async function lookupUserByName(name) {
  try {
    ...
    const match = members.find(m => {
      ...
      // Match if all name parts appear in the real name (handles "Daniel 'D' Sherr" vs "Daniel Sherr")
 ...
 * Requires a user token (xoxp-*) — bot tokens cannot use search.
 *
 * @param {string} query - Slack search query (e.g., "from:<@U123> after:2026-03-16 before:2026-03-23")
 * @param {Object} options - Optional parameters
 * @param {string} options.sort - Sort field (default: 'timestamp')
 * @param {string} options.sort_dir - Sort direction (default: 'desc')
 * @param {number} options.count - Results per page (default: 100, max: 100)
 * @returns {Promise<Array<{text: string, user: string, channel: {id: string, name: string}, ts: string, permalink: string}>>}
 */
async function searchMessages(query, options = {}) {
  const userToken = config.slackUserToken;
  if (!userToken) {
    log.warn('No slackUserToken configured — search.messages requires a user token');
    return [];
  }
  ...
  do {
    ...
    const res = await slackGetWithToken('search.messages', params, userToken);
  ...
  log.info({ query: query.slice(0, 80), matches: allMatches.length, pages: totalPages }, 'Search completed');
  return allMatches;
}
