...
// Cached token + client, refreshed periodically
let cachedToken = null;
let tokenFetchedAt = 0;
let client = null;
const TOKEN_CACHE_TTL = 30 * 60 * 1000; // Re-read credentials every 30 minutes

// LLM Gateway URL — if set, route through the gateway with gcloud auth
const config = require('./config');
const GATEWAY_URL = config.anthropicBaseUrl || null;

/**
 * Get a gcloud identity token for LLM gateway authentication.
...
function readGcloudToken() {
  try {
    ...
    log.warn('gcloud identity token does not look like a JWT');
    return null;
  } catch (err) {
    log.warn({ err: err.message }, 'Could not get gcloud identity token');
    return null;
  }
}
 ...
 * Priority: LLM gateway (gcloud) → macOS keychain → ANTHROPIC_API_KEY env.
...
function getClient() {
  ...
  if (stale) {
    // Strategy 1: LLM Gateway with gcloud identity token
    if (GATEWAY_URL) {
      const token = readGcloudToken();
      if (token && token !== cachedToken) {
        ...
        log.info({ gateway: GATEWAY_URL }, 'AI client initialized via LLM gateway');
      } else if (!token) {
        log.warn('LLM gateway configured but gcloud token unavailable');
      }
      tokenFetchedAt = now;
      return client;
    }

    // Strategy 2: Claude Code keychain OAuth
    ...
    if (token && token !== cachedToken) {
      ...
      // Strategy 3: env vars (ANTHROPIC_API_KEY)
      try {
        client = new Anthropic();
