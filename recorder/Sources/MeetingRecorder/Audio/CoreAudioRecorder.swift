...
// Extracted from VoiceInk (GPL v3) — https://github.com/nicepkg/VoiceInk
final class CoreAudioRecorder {
    ...
    func startRecording(toOutputFile url: URL, deviceID: AudioDeviceID) throws {
        ...
        logger.notice("Starting recording from device \(deviceID)")
    ...
    func switchDevice(to newDeviceID: AudioDeviceID) throws {
        ...
        logger.notice("Switching recording device from \(oldDeviceID) to \(newDeviceID)")

        // Step 1: Stop the AudioUnit (but keep file open)
        var status = AudioOutputUnitStop(unit)
        if status != noErr {
            logger.warning("Warning: AudioOutputUnitStop returned \(status)")
        }

        // Step 2: Uninitialize to allow reconfiguration
        status = AudioUnitUninitialize(unit)
        if status != noErr {
            logger.warning("Warning: AudioUnitUninitialize returned \(status)")
        }
        ...
        logger.notice("Successfully switched to device \(newDeviceID)")
    }
    ...
    private func configureFormats() throws {
        ...
        logger.notice("Device format: sampleRate=\(devSampleRate), channels=\(devChannels), bitsPerChannel=\(devBits)")
        logger.notice("Output format: sampleRate=\(outSampleRate), channels=\(outChannels), bitsPerChannel=\(outBits)")
        if devSampleRate != outSampleRate {
            logger.notice("Converting: \(Int(devSampleRate))Hz -> \(Int(outSampleRate))Hz")
        }
    ...
    private func convertAndWriteToFile(inputBuffer: inout AudioBufferList, frameCount: UInt32) {
        ...
        if writeStatus != noErr {
            logger.error("ExtAudioFileWrite failed with status: \(writeStatus)")
        }
    ...
    private func logDeviceDetails(deviceID: AudioDeviceID) {
        ...
        logger.notice("Device info: name=\(deviceName), uid=\(deviceUID)")
        logger.notice("Device details: transport=\(transportType), manufacturer=\(manufacturer)")

        if let bufferSize = getBufferFrameSize(deviceID: deviceID) {
            let latencyMs = (Double(bufferSize) / 48000.0) * 1000.0
            logger.notice("Buffer size: \(bufferSize) frames, ~latency: \(String(format: "%.1f", latencyMs))ms")
        }
    }
