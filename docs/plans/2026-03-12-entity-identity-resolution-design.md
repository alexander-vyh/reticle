...
time — `findOrCreateEntity(db, 'person', "Gimli 'G' Stone\")`. These entities have
...
Observed: 2 orphaned Gimli Stone entities with 24 facts between them. 89 total person
...
still misses name variants like `"Gimli 'G' Stone"` vs `"Gimli Stone"`; (3) it cannot
...
3. **Merge UI: preferred name + identity review** — when merging two entities, show
   ...
   `Gimli 'G' Stone` into `Gimli Stone` produces the correct canonical name without
...
5. **Retroactive migration of existing floating entities** — script to migrate facts
   ...
   the orphaned Gimli Stone entities have zero facts and can be deactivated.
