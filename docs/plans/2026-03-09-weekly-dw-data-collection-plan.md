...
| Jira activity | `ENG` and `ENGSUP` projects — ticket transitions, comments, new issues | `raw_messages` with `source='jira'` |
...
assert.ok(jql.includes('project in (ENG, ENGSUP)'), 'Should include projects');
...
assert.strictEqual(msg.channel_name, 'ENG');
```
...
Create a scheduled collector that polls Jira for recent activity in `ENG` and `ENGSUP` and writes it to `raw_messages`. This runs on a schedule (daily or more frequently).
...
// 3. Build JQL: project in (ENG, ENGSUP) AND updatedDate >= -1d
...
git commit -m "feat: add Jira activity collector for ENG and ENGSUP"
```
