...
const prompt = buildMeetingSummaryPrompt({
  ...
  attendees: ['Alexander Vyhmeister', 'Mark Johnson'],
  title: 'Weekly Standup',
  durationMin: 30
});
...
assert.ok(prompt.userMessage.includes('Mark Johnson'));
...
match "Mark" to the attendee named Mark Johnson if present.
...
reticleDb.saveMeetingSummary(db, {
  ...
  keyPeople: [{ mentioned: 'Mark', resolvedName: 'Mark Johnson', context: 'AI policy owner' }],
  flaggedItems: [
