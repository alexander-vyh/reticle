...
// ══════════════════════════════════════════════════════════════════════════
{
  const { db, path: p } = tmpDb();
  try {
    const entity = kg.createEntity(db, { entityType: 'person', canonicalName: 'Daniel Sherr' });
    kg.addIdentity(db, { entityId: entity.id, source: 'slack', externalId: 'U_DAN', displayName: 'Daniel Sherr' });
    kg.addIdentity(db, { entityId: entity.id, source: 'jira', externalId: 'dan-jira', displayName: 'Dan Sherr' });
    ...
    assert.ok(aliasValues.includes('Daniel Sherr'), 'should include canonical_name / display_name');
    assert.ok(aliasValues.includes('Dan Sherr'), 'should include display_name from jira');
...
// ══════════════════════════════════════════════════════════════════════════
{
  ...
  try {
    ...
    const entity = kg.createEntity(db, { entityType: 'person', canonicalName: 'Daniel Sherr' });
    kg.addIdentity(db, { entityId: entity.id, source: 'slack', externalId: 'U_DAN', displayName: 'Daniel Sherr' });
    kg.addAlias(db, { entityId: entity.id, alias: 'Daniel Sherr', aliasSource: 'canonical_name' });

    // Insert a raw message from Daniel
    kg.insertRawMessage(db, {
      source: 'slack', sourceId: 'slack:msg1', channelName: 'general',
      authorExtId: 'U_DAN', authorName: 'Daniel Sherr',
    ...
    kg.upsertFact(db, {
      entityId: null, mentionedName: 'Daniel Sherr',
      attribute: 'committed_to', value: 'ship the widget by Friday',
      factType: 'event', sourceMessageId: msg.id,
    });

    // Verify fact is unattributed
    const before = db.prepare('SELECT entity_id FROM facts WHERE mentioned_name = ?').get('Daniel Sherr');
    ...
    const after = db.prepare('SELECT entity_id FROM facts WHERE mentioned_name = ?').get('Daniel Sherr');
...
// ══════════════════════════════════════════════════════════════════════════
{
  ...
  try {
    ...
    const entity = kg.createEntity(db, { entityType: 'person', canonicalName: 'Bill Price' });
    kg.addIdentity(db, { entityId: entity.id, source: 'slack', externalId: 'U_BILL', displayName: 'Bill Price' });
    kg.addAlias(db, { entityId: entity.id, alias: 'Bill Price', aliasSource: 'canonical_name' });

    // Deferred fact without a source_message_id
    kg.upsertFact(db, {
      entityId: null, mentionedName: 'Bill Price',
    ...
    const fact = db.prepare("SELECT entity_id FROM facts WHERE mentioned_name = 'Bill Price'").get();
...
// ══════════════════════════════════════════════════════════════════════════
{
  const { db, path: p } = tmpDb();
  try {
    const entity = kg.createEntity(db, { entityType: 'person', canonicalName: 'Ken Dominiec' });
    kg.addIdentity(db, { entityId: entity.id, source: 'slack', externalId: 'U_KEN', displayName: 'Ken Dominiec' });
    kg.addAlias(db, { entityId: entity.id, alias: 'Ken Dominiec', aliasSource: 'canonical_name' });
...
// ══════════════════════════════════════════════════════════════════════════
{
  ...
  try {
    ...
    const entity = kg.createEntity(db, { entityType: 'person', canonicalName: 'Kinski Wu' });
    kg.addIdentity(db, { entityId: entity.id, source: 'slack', externalId: 'U_KINSKI', displayName: 'Kinski Wu' });
    kg.addAlias(db, { entityId: entity.id, alias: 'Kinski Wu', aliasSource: 'canonical_name' });
    ...
    kg.upsertFact(db, {
      entityId: null, mentionedName: 'Kinski Wu',
    ...
    // "Kinski Wu" should be attributed (Path B), "K. Wu" should not
    assert.strictEqual(metrics.sweepPathBMatched, 1);
